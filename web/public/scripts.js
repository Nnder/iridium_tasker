        ...
        <div class="modal-content" style='padding:5px; height:100%; background-color: rgb(247, 247, 247);'>
		  ...
		  <form id="feedback-form" action="">
			  ...
			  <div class='row' style='padding-top:5px'>
				...
				<input class='form-control' id='hours' type='text' style='width:50px; padding:4px' name="hoursedit" value='${$(this).attr('hours').slice(0, 5) ?? ''} '>
...
$(document).on('click', '#team_traveling', function() {
  for (let member in travel_arr) delete travel_arr[member];
  $.ajax({
    type: "POST",
    url: 'ajax.php',
...
var travel_arr = new Object();
$(document).on('click', '#team_change', async function() {
  await $(".team_checkbox:checked").each(function(){
    travel_arr[$(this).closest('.row').find('input[type=text]').attr('number_phone')] = $('#team_combobox').val();
  ...
  if (await !$(".team_checkbox").length) {
    $.ajax({
      type: "POST",
      url: 'ajax.php',
      data: {team_change: travel_arr, title: $(this).closest('.row').attr('title')},
      success: function (data) {
        eval(data);
      }
    });
...
//...............Редактирование сотрудника (команды)
$(document).on('click', '#team', function () {
  $.ajax({
    type: "POST",
    url: "ajax.php",
    data: {edit_team: $('#team').val()},
    success: function (data) {
      eval(data);
    }
  });;
...
//.............Отмена редактирования команд................
$(document).on('click', '#edit_team_cancel', function () {
  but_emp(edit_number);
});

//.............Подтверждение редактирования команд................
$(document).on('click', '#edit_team_change', function () {
  let checked_arr = [];
  let text;
  if ($('#edit').find('input[type=checkbox]:checked').length) {
    $('#edit').find('input[type=checkbox]:checked').each(function() {
      checked_arr.push($(this).closest('.row').find('input[type=text]').val());
    });
    text = checked_arr.toString();
  } else {
    text = 'Без команды';
  }
  but_emp(edit_number);
  waitForElm('#team').then(() => {
    $('#team').val(text);
  });
});
