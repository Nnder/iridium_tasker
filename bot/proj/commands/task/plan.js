...
async function startPlan(chat_id) {
        ...
        [
          {
            ...
            callback_data: JSON.stringify({ type: "Enter Plan", chat_id }),
          },
          {
            text: "Не работаю",
            callback_data: JSON.stringify({ type: "Not Work", chat_id }),
          },
        ],
      ],
    },
  ...
  const messageWithKeyboard = await bot.sendMessage(
    chat_id,
    "Доброе утро! Что на сегодня запланировал?",
    options,
  );
...
async function timestampWork(chat_id, minus = 0) {
  ...
  return (
    1000 * 60 * 60 * time.getHours() + 1000 * 60 * time.getMinutes() - minus
  );
}
